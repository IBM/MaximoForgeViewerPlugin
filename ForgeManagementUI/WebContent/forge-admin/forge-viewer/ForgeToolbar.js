...
IBM.LMV.toolBarExtension = function(
	...
	this.onCreate = function( e )
	{
		...
		var appSubMenu = new Autodesk.Viewing.UI.ComboButton( IBM.LMV.ToolBar.ID_TOOLBAR_MAXIMO );
		...
		appSubMenu.addControl( buttonMaximoOpt );
...
IBM.LMV.SaveViewDlg = function (
	...
	this.saveView = function()
	{
		...
		var that = this;
		var form = "";
		form = form + "urn"          + "=" + this.forgeViewer.docURN;
		form = form + "&description" + "=" + this.descInput.value;
//		form = form + "&viewerstate" + "=" + this.viewerState;
		form = form + "&viewerstate" + "=" + 200;
		if( this.detailsInput.value && this.detailsInput.value != "" )
		{
			form = form + "&detail"  + "=" + this.detailsInput.value;
		}
		if( this.sharedCB.checked === true )
		{
			form = form + "&shared=true";
		}
		
		var len = this.viewerState;
		var bytes = new Uint8Array( len );
		for (var i = 0; i < len; i++)        
		{
			bytes[i] = this.viewerState(i);
		}
		
		var url = IBM.LMV.Auth.getRestURL();
		url = "/dev/api/p/v1/forge/view";
		var xmlReq = new XMLHttpRequest();
	...
	*/
	...
	this.saveView = function()
	{
		...
		body.data.description = this.descInput.value;
		body.data.detail      = this.detailsInput.value;
		body.data.urn         = this.forgeViewer.docURN;
		body.data.view        = stateJSON;
		body.data.viewerState = bytes;
		body.data.shared      = this.sharedCB.checked;
		body.data.user        = this.forgeViewer.currentUser._userAccount;
...
//		var decoded = atob( view.view );
		var state = JSON.parse( view.view );
